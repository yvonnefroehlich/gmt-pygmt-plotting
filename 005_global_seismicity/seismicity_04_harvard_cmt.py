...
for depth_min, depth_max in zip(
    ...
    # Shift plot origin
    fig.shift_origin(xshift=f"+w{x_shift}c")
    if depth_min == 20:
        fig.shift_origin(
    ...
    fig.show()
fig_name = f"map_globalseis_{year_min}to{year_max}_meca_fault_{fault_intval}deg"
...
for depth_min, depth_max in zip(
    ...
    # Plot epicenters as beachballs
    for i_df, df_eq_dist in enumerate([df_eq_close, df_eq_far, df_eq_xks]):
        meca_fill = "tan"
        if i_df == 2:
            meca_fill = color_hl
        if len(df_eq_dist) > 0:
    ...
    # Show and save figure
    fig.show()
fig_name = f"map_globalseis_{year_min}to{year_max}_meca_xks"
for ext in ["png"]:  # "pdf", "eps"
    fig.savefig(fname=f"{path_out}/{fig_name}.{ext}", dpi=dpi_png)
print(fig_name)
...
for i_mag, lim_mag in enumerate(np.arange(min_mag, max_mag, step_mag)):  # [ [
    ...
    if i_mag == 4:  # Add colorbar only once
        with gmt.config(FONT="15p"):
            fig.colorbar(frame=cmap_label, position="+e0.25c")
    ...
    # Show and save figure
    fig.show()
fig_name = f"map_globalseis_{year_min}to{year_max}_meca_{quantity_for_color}"
for ext in ["png"]:  # "pdf", "eps"
    fig.savefig(fname=f"{path_out}/{fig_name}.{ext}", dpi=dpi_png)
print(fig_name)
